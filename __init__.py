...
def load_ethereum_data(filename):

    data = pd.read_csv(filename)

    timestamps = set(data['block_number'].unique())
    timestamp_counter = 0
    print(timestamps)
    node2idx = {}
    node_counter = 0

    for address in data['token_address'].unique():
        if address not in node2idx:
            node2idx[address] = node_counter
            node_counter += 1

    for address in data['from_address'].unique():
        if address not in node2idx:
            node2idx[address] = node_counter
            node_counter += 1

    for address in data['to_address'].unique():
        if address not in node2idx:
            node2idx[address] = node_counter
            node_counter += 1
    ...
    for index, row in data.iterrows():
        ...
        token_idx = node2idx[token]
        u_idx = node2idx[u]
        v_idx = node2idx[v]
        timestamp_idx = timestamp2idx[block_number]

        L[timestamp_idx, u_idx, v_idx] += value
        b[timestamp_idx, u_idx] += value
        c[timestamp_idx, v_idx] += value
